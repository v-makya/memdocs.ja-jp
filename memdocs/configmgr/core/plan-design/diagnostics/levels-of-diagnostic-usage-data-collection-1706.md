...
description: Configuration Manager バージョン 1706 で収集される診断結果および使用状況データの各種レベルについて説明します。
ms.date: 07/31/2017
...
Configuration Manager バージョン 1706 では、**基本**、**エンハンス**、**フル**の 3 つのレベルの診断結果と使用状況データが収集されます。 既定では、この機能は、エンハンス レベルに設定されます。 以降のセクションでは、各レベルで収集されるデータについて詳しく説明します。
...
Configuration Manager バージョン 1706 では、このレベルには次のデータが含まれます。
...
- ***[更新]*** サイト システム サーバーの基本情報 (使用されるサイト システムの役割、インターネットと SSL ステータス、オペレーティング システム、プロセッサ、物理または仮想マシン、高可用性のサイト サーバーの使用)
...
- ***[新規]*** 管理コンソールのプロパティ ページとウィザードへの拡張機能のハッシュされた一覧
...
- **アプリケーション管理:**  
   ...
   - ***[新規]*** Windows インストーラーの展開の種類の数 (アンインストール コンテンツの設定別)
...
- **Cloud Services:**
  ...
  - ***[新規]*** Azure Active Directory の検出に関する統計情報

  - ***[更新]*** リージョンと環境の数、認証と承認の統計情報など、クラウド管理ゲートウェイの構成と使用の統計情報

  - ***[新規]*** Configuration Manager に接続されている Azure Active Directory アプリケーションとサービスの数
...
- **コンテンツ:**  
    ...
    - ***[更新]*** ピア キャッシュ クライアントの数、使用状況の統計情報、および部分的なダウンロードの統計情報
...
- **オペレーティング システムの展開:**  
    ...
    - ***[新規]*** Configuration Manager クライアント バージョン別のブート イメージの数

    - ***[新規]*** Windows PE バージョン別のブート イメージの数

    - エディション アップグレード ポリシーの数

    - ***[新規]*** PXE から除外されたハードウェア識別子の数

    - ***[新規]*** コンテンツを事前にダウンロードするオプションを使用したタスク シーケンスの展開の数

    - タスク シーケンス ステップの使用数

    - ***[新規]*** インストールされている Windows ADK のバージョン
...
- **ソフトウェア更新プログラム:**  
    ...
    - ***[新規]*** 作成および展開されている Windows Update for Business ポリシーの数
...
- ***[新規]*** コンプライアンスとエラーでのコンプライアンス ポリシーの統計情報
