---
title: 1906 での診断情報と使用状況データ
titleSuffix: Configuration Manager
description: バージョン 1906 で Configuration Manager によって各レベルで収集される特定のデータについて説明します。
ms.date: 07/26/2019
...
Configuration Manager バージョン 1906 では、このレベルには次のデータが含まれます。

- ***[更新]*** Configuration Manager コンソール接続に関する統計情報: OS のバージョン、言語、SKU とアーキテクチャ、システム メモリ、論理プロセッサの数、接続サイト ID、インストールされた .NET のバージョン、コンソールの言語パック、可能な認証レベル
...
- ***[新規]*** キー サイト属性のハッシュ (サイト ID、SQL ブローカー ID、サイト交換キー)
...
- ***[新規]*** アプリケーション グループの承認機能の使用に関して集計された統計情報

- ***[新規]*** Office アドインで集計された統計情報、Office 準備ツールキットの使用状況、Microsoft 365 Apps for enterprise でのクライアントの数
...
- ***[新規]*** Azure Active Directory に同期されたコレクションの数
...
- ***[新規]*** PXE とクライアント登録から除外された重複するハードウェア識別子 (MAC アドレスと SMBIOS GUID) の数

- ***[新規]*** 種類別のタス クシーケンスの数 (OS の展開または汎用タスク シーケンス)

- ***[新規]*** 事前キャッシュ コンテンツ設定でのパッケージの数
...
- ***[新規]*** サードパーティのソフトウェア更新プログラム カタログのサブスクリプションのリスト
...
- ***[新規]*** 管理ポイント上のソフトウェア センター エンドポイントに対して行われた呼び出しの集計されたパフォーマンス統計情報
...
- ***[新規]*** Desktop Analytics の承認機能の使用に関して集計された統計情報

- ***[新規]*** SQL メンテナンス タスクの構成と状態
...
- ***[新規]*** Configuration Manager コンソール拡張機能のリスト
