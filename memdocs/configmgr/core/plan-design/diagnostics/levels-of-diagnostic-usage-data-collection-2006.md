---
title: 2006 での診断情報と使用状況データ
titleSuffix: Configuration Manager
description: バージョン 2006 で Configuration Manager によって各レベルで収集される特定のデータについて説明します。
ms.date: 08/11/2020
...
ms.locfileid: "88129587"
---
# <a name="diagnostic-and-usage-data-for-version-2006"></a>バージョン 2006 の診断情報と使用状況データ
...
Configuration Manager バージョン 2006 では、このレベルには次のデータが含まれます。
...
- Configuration Manager サイト階層の基本データ: サイトのリスト、種類、バージョン、状態、クライアントの数、タイム ゾーン、正常性状態
...
- Configuration Manager コンソールのクラッシュの場所

- Configuration Manager コンソールの使用状況の統計

- クラウドのアタッチおよびデタッチのアクション

- Intune クラウド サービスとの最後の同期の状態

- 管理サービスからのエラー数

- 一括登録トークンの使用

- 既定と優先ブラウザー別のクライアントの数

- "***移動***" クラウド管理ゲートウェイの構成と使用状況の統計情報: リージョンと環境の数、認証/承認の統計情報  

- "***移動***" SQL AlwaysOn レプリカ情報、使用状況、正常性の状態  

- "***新規***" 管理コンソールの通知の構成と状態

- "***新規***" サイトの正常性チェックの構成と状態
...
- ダッシュボードを使用して作成された Microsoft 365 Apps アプリケーションの数  
...
- Office アドインで集計された統計情報、Office 準備ツールキットの使用状況、Microsoft 365 Apps でのクライアントの数

- Office アドインの正常性に関して集計された統計情報

- Office Pro Plus パイロット コレクションの数とサイズ

- Office の正常性データを送信する Office Pro Plus デバイスの数
...
- "***更新***" - "***新規***" ソフトウェア インベントリ ルール、ファイル収集規則、および全体的な正常性状態
...
- "***新規***" カテゴリ、OS、ソース別の展開されたコンプライアンス設定の数 (クラウドとオンプレミス)
...
- コンソールでアクセスされる最も一般的な 25 件のアクション、ウィザード、プロパティ シート、ツリー ノード
...
- Microsoft Defender Advanced Threat Protection ポリシーに関して集計された統計情報
...
- 展開された Microsoft 365 Apps 更新プログラムの数  
...
- オーケストレーション グループの使用状況

- Windows Update フォールバック構成設定
