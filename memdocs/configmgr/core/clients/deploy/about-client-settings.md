...
**[はい]** を選択すると、クライアントでの配信最適化グループ識別子として境界グループ識別子が適用されます。 クライアントは、配信の最適化クラウド サービスと通信するとき、この識別子を使って目的のコンテンツを含むピアを探します。 また、この設定を有効にすると、配信の最適化ダウンロード モードも、対象クライアントのグループ (2) オプションに設定されます。
