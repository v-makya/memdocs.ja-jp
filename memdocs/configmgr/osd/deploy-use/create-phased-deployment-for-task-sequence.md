...
> [!Tip]  
> 段階的展開機能はバージョン 1802 で[プレリリース機能](../../core/servers/manage/pre-release-features.md)として初めて導入されました。 バージョン 1806 からは、この機能はプレリリース機能ではなくなります。<!--1356837-->  
...
- **正常に展開されたデバイスの数**: Configuration Manager バージョン 1902 に追加されています。 最初のフェーズが成功するために展開を正常に完了する必要のあるデバイスの数を指定します。 このオプションは、コレクションの数が変動し、次のフェーズに移る前に特定の数の成功したデバイスを示す必要がある場合に役立ちます。 <!--3555946-->
...
1. Configuration Manager コンソールで段階的な展開の作成ウィザードを開始します。 このアクションは、展開しているソフトウェアの種類によって異なります。  
    ...
    - **ソフトウェア更新プログラム** (バージョン 1810 以降でのみ): **[ソフトウェア ライブラリ]** に移動し、 **[ソフトウェア更新プログラム]** を展開して、 **[すべてのソフトウェア更新プログラム]** を選択します。 1 つまたは複数の更新プログラムを選択してから、リボンで **[段階的な展開の作成]** を選びます。  
...
3. 段階的展開の作成ウィザードの **[フェーズ]** ページでは、次のアクションを使用できます。  
    ...
    - 新しいフェーズの **[追加]** :  
        ...
        2. フェーズの追加ウィザードの **[フェーズの設定]** ページで、スケジュールの設定を構成し、完了したら **[次へ]** を選択します。 詳しくは、「[設定](#bkmk_settings)」をご覧ください。   
            ...
            > [!Note]  
            > 最初のフェーズのフェーズの設定 **[展開の成功率]** または **[正常に展開されたデバイスの数]** (バージョン 1902 以降) を編集することはできません。 この設定は、前にフェーズがあるフェーズにのみ適用されます。  
