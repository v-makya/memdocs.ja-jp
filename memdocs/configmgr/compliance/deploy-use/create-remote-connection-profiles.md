...
- ユーザーがインターネットから接続できるようにするには、リモート デスクトップ ゲートウェイ サーバーをインストールして構成します。 リモート デスクトップ ゲートウェイ サーバーをインストールして構成する方法の詳細については、「[リモート デスクトップ サービスのどこからでもアクセス](/windows-server/remote/remote-desktop-services/rds-plan-access-from-anywhere)」を参照してください。
...
1. **[プロファイルの設定]** ページで、次の設定を指定します。  
    ...
    - **ネットワーク レベル認証によるリモート デスクトップを実行するコンピューターからの接続のみを許可する**:既定で有効になります。この設定により、接続のセキュリティ レベルが追加されます。 詳細については、「[リモート デスクトップ アクセスを許可する](/windows-server/remote/remote-desktop-services/clients/remote-desktop-allow-access#why-allow-connections-only-with-network-level-authentication)」を参照してください。
