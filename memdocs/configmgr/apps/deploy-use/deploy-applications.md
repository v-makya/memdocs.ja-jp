...
- イントラネット上のクライアントをサポートするには  
  ...
  > [!NOTE]
  > イントラネット上で検出されているものの、Configuration Manager バージョン 2002 以前のクラウド管理ゲートウェイ (CMG) を介して通信しているクライアントの場合、ソフトウェア センターでは Windows 認証が使用されます。 ユーザーの使用可能なアプリの一覧を CMG 経由で取得しようとすると、失敗していました。 バージョン 2006 以降では、Azure AD に参加しているデバイスでは、Active Directory (Azure AD) ID が使用されます。 これらのデバイスは、クラウド参加もハイブリッド参加も可能です。<!--6935376-->
