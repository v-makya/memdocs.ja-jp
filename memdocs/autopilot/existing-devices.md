...
3. 次の行を入力し、Intune の管理者資格情報を指定します。
   - 指定したユーザーアカウントに十分な管理者権限があることを確認してください。
     ...
     Intune Graph Api を初めて使用する場合は、Microsoft Intune PowerShell の読み取りと書き込みのアクセス許可を有効にするように求められます。 これらのアクセス許可を有効にするには、次の手順を実行します。
   - **代理または組織の同意**を選択する
...
4. 次に、指定した Intune テナントで使用可能なすべての自動操縦プロファイルを JSON 形式で取得して表示します。
   ...
   |           CloudAssignedTenantId (guid、必須)           | 使用する必要がある Azure Active Directory テナント ID。 このプロパティはテナントの GUID であり、テナントの [プロパティ] にあります。 値に中かっこを含めることはできません。                                                                                       |
   |        CloudAssignedTenantDomain (文字列、必須)        | 使用する必要がある Azure Active Directory テナント名 (例: tenant.onmicrosoft.com)。                                                                                                                                  |
   |         CloudAssignedOobeConfig (number, required)         | このプロパティは、構成された自動操縦設定を示すビットマップです。 値は次のとおりです: SkipCortanaOptIn = 1、OobeUserNotLocalAdmin = 2、SkipExpressSettings = 4、SkipOemRegistration = 8、SkipEula = 16                                                                           |
   ...
   |      CloudAssignedForcedEnrollment (number, required)      |                                                                                                                         デバイスで Azure AD 参加と MDM 登録が必要であることを指定します。 <br>0 = 必須ではありません。 1 = 必須です。                                                                                                                         |
   ...
   | CloudAssignedAadServerData (エンコードされた JSON 文字列、必須) |                                                  ブランド化に使用される埋め込み JSON 文字列。 Azure AD corp ブランド化が有効になっている必要があります。 <br> 値の例: "CloudAssignedAadServerData": "{ \" ZeroTouchConfig \" : { \" CloudAssignedTenantUpn \" : \" \" , \" CloudAssignedTenantDomain \" : \" tenant.onmicrosoft.com \" }}"                                                   |
   |         CloudAssignedDeviceName (文字列、省略可能)         | コンピューターに自動的に割り当てられる名前。 これは、Intune の自動操縦プロファイルで構成されている名前付けパターン規則に従います。 または、使用する明示的な名前を指定することもできます。 |


5. 自動操縦プロファイルは、ASCII または ANSI 形式で JSON ファイルとして保存する必要があります。 Windows PowerShell の既定値は Unicode 形式です。 そのため、コマンドの出力をファイルにリダイレクトする場合は、ファイル形式も指定します。 たとえば、Windows PowerShell を使用してファイルを ASCII 形式で保存するには、ディレクトリを作成し (例: C:\ 自動操縦)、次のようにプロファイルを保存します (コマンド文字列全体を表示するには、下部にある水平スクロールバーを使用します)。

    ```powershell
    Get-AutopilotProfile | ConvertTo-AutopilotConfigurationJSON | Out-File c:\Autopilot\AutopilotConfigurationFile.json -Encoding ASCII
    ```
    **重要**: ファイル名 ** にはAutopilotConfigurationFile.js** という名前を付け、ASCII/ANSI としてエンコードする必要があります。 
...
4. [ **次へ**] をクリックし、次の **メンバーシップ規則** の詳細を入力します。
   ...
   - たとえば、ワイプして再読み込みするコンピューターのホスト名が PC-01 で、属性として名前を使用する場合は、次のようになります。
      1. [> 規則の追加] をクリックして **> (ウィザードが開きます) > 次へ**] をクリックします。
      2. [**値**] の横に「 **PC-01」と**入力します。
      3. [**次**  >  **の PC-01** ] ([**リソース**] の下) をクリックします。 次の例を参照してください。

         ![[リソースの検索] ダイアログボックス ](images/pc-01a.png)
          ![ [リソースの選択] ダイアログボックス](images/pc-01b.png)
...
4. タスクシーケンスの作成ウィザードで、次の情報を入力します。
   ...
   - [**次**の参照] をクリックし  >  **Browse**て、Windows 10**イメージパッケージ**と**イメージインデックス**バージョン1803以降を選択 > ます。
...
8. [アプリケーションのインストール] ページで、必要に応じてアプリケーションを追加できます。
...
16. [**既存のデバイス構成ファイルの自動操縦を適用**する] に**名前**を変更し、次のコードを [>**適用**する] の [**コマンドライン**] テキストボックスに貼り付けます。
    ```
...
17. [**既存のデバイスの自動操縦の適用] 構成ファイル**のステップ**で、[** 参照] を選択し  >  **Browse**ます。
...
22. [ **自動操縦用デバイスの準備** ] グループがタスクシーケンスの最後のステップであることを確認します。 必要に応じて [ **下へ移動** ] ボタンを使用します。
...
24. 2番目の手順を追加するには、[ **追加**] をクリックし、[ **イメージ**] をポイントし、[ **Windows のキャプチャの準備**] をクリックします。 この手順では、次の設定を使用します。
    ...
    - <u>アクティブ化フラグをリセット</u>しません: 選択され **ていません**
...
> [!NOTE]
> Windows 10 1903 および1909では、の **AutopilotConfigurationFile.jsは [** **windows のキャプチャの準備** ] ステップによって削除されます。 詳細と回避策については、「 [Windows 自動操縦に関する既知の問題](known-issues.md)」を参照してください。
...
4. 配布ポイントの追加ウィザードまたは配布ポイントグループの追加ウィザードで、タスクシーケンスが JSON ファイルを取得できるように、コンテンツの宛先を指定します。
...
2. ソフトウェアの展開ウィザードで、次の**全般的な****設定と展開の設定**の詳細を入力します。
    ...
    - <u>操作</u>: **インストール**します。
    - <u>目的</u>: **使用可能**。 必要に応じて、[**使用可能**な代わりに**必須**] を選択できます。 偶発的な構成が悪影響を及ぼす可能性があるため、この設定はテスト中には推奨されません。
    ...
    - <u>ローカルの配布ポイントを使用できない場合は、リモートの配布ポイントを使用</u>する: 省略可能。
...
1. 対象の Windows 7 または Windows 8.1 クライアントコンピューターで、[スタート] をクリックし > 「 **ソフトウェアセンター** 」と入力し > enter キーを押します。

2. [ソフトウェアライブラリ] で、[ **既存のデバイスの自動操縦** ] を選択し、[ **インストール**] をクリックします。 次の例を参照してください。

    ![[既存のデバイスの自動操縦] ページの ](images/sc.png) ![ 確認ダイアログボックス](images/sc1.png)

タスクシーケンスは次のようになります。
1. コンテンツのダウンロード
2. デバイスを再起動する
3. ドライブのフォーマット
4. Windows 10 をインストールする
5. 自動操縦を準備する

タスクシーケンスが完了すると、デバイスは OOBE で起動し、自動操縦エクスペリエンスを提供します。
...
>[!NOTE]
>デバイスを Active Directory (Hybrid Azure AD Join) に参加させる場合は、"すべてのデバイス" を対象とするドメイン参加デバイス構成プロファイルを作成する必要があります (グループベースのターゲット設定を実行するコンピューターには、Azure Active Directory デバイスオブジェクトが存在しないため)。 詳細については、「 [ハイブリッド Azure Active Directory 参加のユーザー駆動モード](user-driven.md#user-driven-mode-for-hybrid-azure-active-directory-join)」を参照してください。

### <a name="register-the-device-for-windows-autopilot"></a>Windows 自動操縦用デバイスの登録

自動操縦によってプロビジョニングされたデバイスでは、最初の起動時にガイド付きの OOBE 自動操縦エクスペリエンスが提供されます。 Windows 10 に更新した後、PC がリセットされたときに自動操縦機能が使用されるように、デバイスを登録してください。 割り当てられたグループの自動登録を有効にするには、[ **すべての対象デバイスを自動操縦に変換する** ] 設定を使用します。 詳しくは、「[Autopilot Deployment プロファイルを作成する](enrollment-autopilot.md#create-an-autopilot-deployment-profile)」をご覧ください。
