...
ms.locfileid: "83990589"
---
# <a name="use-shell-scripts-on-macos-devices-in-intune"></a>Intune で macOS デバイスに対してシェル スクリプトを使用する
...
6. **[割り当て]**  >  **[含めるグループを選択]** の順に選択します。 Azure AD グループの既存のリストが表示されます。 スクリプトを受信する、1 つまたは複数のユーザーもしくはデバイス グループを選択します。 **[選択]** を選択します。 選択したグループがリストに表示され、スクリプト ポリシーが与えられます。
   > [!NOTE]
   > - ユーザー グループに割り当てられたシェル スクリプトは、Mac にログインしているすべてのユーザーに適用されます。  
   > - シェル スクリプトの割り当て更新では、[Microsoft Intune MDM Agent for macOS](macos-shell-scripts.md#microsoft-intune-management-agent-for-macos) の割り当ても更新されます。
...
これには、次のような理由があります。
* 新しいスクリプトや更新後のスクリプトを受け取るには、エージェントのチェックインが必要になることがあります。 このチェックイン プロセスは 8 時間ごとに実行され、MDM チェックインとは異なります。 エージェントのチェックインを成功させるには、デバイスが起動しており、ネットワークに接続されていることを確認します。そして、エージェントがチェックインするのを待ちます。 また、Mac でポータル サイトを開き、デバイスを選択して **[設定の確認]** をクリックするようにエンド ユーザーに要求することもできます。
...
> [!NOTE]
> Microsoft Endpoint Manager 管理コンソールでのデバイスの**同期**アクションでは、MDM チェックインが開始され、エージェントのチェックインは強制されません。
 ...
 ### <a name="why-are-scripts-running-even-though-the-mac-is-no-longer-managed"></a>Mac が管理対象から外された場合でも、スクリプトが実行されるのはなぜですか。
 スクリプトが割り当てられた Mac が管理対象から外された場合、エージェントは直ちに削除されるわけではありません。 次回のエージェント チェックイン時に (通常は 8 時間ごと)、Mac が管理対象から外されたことがエージェントによって検出され、スケジュールされたスクリプトの実行が取り消されます。 そのため、スケジュールされた次回のエージェント チェックインよりも頻繁に実行されるようにスケジュールされている、ローカルに保存されたスクリプトは実行されます。 エージェントがチェックインできない場合は、最大 24 時間 (デバイスが起動している時間) の間、チェックインが再試行され、その後 Mac から削除されます。
