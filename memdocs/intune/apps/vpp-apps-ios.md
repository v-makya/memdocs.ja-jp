...
| アプリの自動更新 | Intune 管理者が、アプリの [割り当ての種類] が [必須] になっている Apple VPP トークンの設定で構成したようになります。<p>[割り当ての種類] が [登録済みデバイスで使用可能] である場合は、利用可能なアプリの更新プログラムをポータル サイトからインストールできます。 | エンド ユーザーが、App Store の個人用設定で構成したようになります。 Intune 管理者がこれを管理することはできません。 |
...
4. **[VPP トークンの作成]** ウィンドウで、次の情報を指定します。
    ...
    - **アプリの自動更新** - **[オン]** で **[オフ]** を選択し、自動更新を有効/無効にします。 有効にすると、アプリ ストア内の VPP アプリ更新プログラムが Intune によって検出され、デバイスのチェックイン時に自動的にデバイスにプッシュされます。
        ...
        > [!NOTE]
        > Apple VPP アプリの自動アプリ更新により、**必須**インストール インテントでデプロイされたアプリのみが自動的に更新されます。 **利用可能**インストール インテントで展開されたアプリでは、自動更新により、新しいバージョンのアプリが利用できることを IT 管理者に知らせるステータス メッセージが生成されます。 このステータス メッセージは、アプリを選択し、[デバイスのインストール状態] を選択して、[状態の詳細] をオンにすることにより、表示できます。  
