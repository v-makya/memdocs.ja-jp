...
Intune で既にプロファイルが展開された後にログを取得する場合、エンド ユーザーがデバイス上のメール アプリを使用して、ご自身にログをメールで送信することができます。 Zebra デバイスでポータル サイト アプリを開き、[ログを送信します](https://docs.microsoft.com/user-help/send-logs-to-your-it-admin-by-email-android)。 ログの送信機能を使用すると、PowerLift インシデント ID も作成されます。これは、Microsoft サポートに問い合わせる場合に参照することができます。
