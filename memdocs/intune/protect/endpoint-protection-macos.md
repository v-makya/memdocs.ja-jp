...
- **FileVault を有効にする**  
  ...
  **既定値**:未構成  
  ...
  - **回復キーの種類**  
    ...
    - **[個人用回復キーのエスクローの場所に関する説明]** - 個人用回復キーを取得できる方法と場所をユーザーに対して説明する短いメッセージを指定します。 このテキストは、ユーザーがログイン画面でパスワードを忘れた場合に表示される個人用回復キーの入力をユーザーに求めるメッセージに挿入されます。  
...
Intune での FileVault の詳細については、「[FileVault の管理](../protect/encrypt-devices-filevault.md#manage-filevault)」を参照してください。
...
- **次のアプリの着信接続を許可する**: 着信接続の受信を許可するアプリを選択します。 次のようなオプションがあります。
...
- **[次のアプリの着信接続をブロックする]** : 着信接続の受信を許可しないアプリを選択します。 次のようなオプションがあります。
  - **[バンドル ID でアプリを追加]** : アプリの[バンドル ID](../configuration/bundle-ids-built-in-ios-apps.md) を入力します。 Apple の Web サイトに、[組み込みの Apple アプリ](https://support.apple.com/HT208094)の一覧があります。
  - **[ストア アプリの追加]** :Intune に以前に追加したストア アプリを選択します。 詳しくは、「[Microsoft Intune にアプリを追加する](../apps/apps-add.md)」をご覧ください。
