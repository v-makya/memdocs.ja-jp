...
description: Microsoft Intune で、会社のリソースにアクセスして MDM 管理のデバイスを使用するときに発生するエラー、状態コード、説明、解決策の一覧、iOS/iPadOS デバイスのエラーや OMA 応答エラーの一覧を参照してください。
keywords: ''
...
## <a name="errors-returned-by-iosipados-devices"></a>iOS/iPadOS デバイスによって返されるエラー
...
|__問題が発生しました__ <br>ポータル サイト クライアントを読み込むことができません。 再試行し、それでもこの問題が解決しない場合は IT 管理者に問い合わせてください。|400 エラー|より具体的なエラー メッセージがない 400 番台の HTTP 状態コードのエラーの場合、このメッセージが表示されます。 これは、iOS/iPadOS 用ポータル サイト アプリで発生するクライアント側エラーです。|
...
|-2016344011|0x87D10835|DCMO(1401):確認メッセージが表示されたときに、ユーザーが処理を受け入れないことを選択しました|
|-2016344012|0x87D10834|DCMO(1400):クライアント エラー|
|-2016344108|0x87D107D4|DCMO(1204):デバイスの機能が無効になっています。ユーザーはこれを再度有効にすることができます|
|-2016344109|0x87D107D3|DCMO(1203):デバイスの機能が無効になっています。ユーザーはこれを再度有効にすることはできません|
...
|-2016345599|0x87D10201|Syncml(513):SyncML 要求メッセージで使用される指定されたバージョンの SyncML 同期プロトコルを、受信者がサポートしていないか、サポートすることを拒否しました。|
...
|-2016345604|0x87D101FC|Syncml(508):クライアントのサーバーとの同期状態を更新する必要のあるエラーが発生しました。|
|-2016345605|0x87D101FB|Syncml(507):エラーが発生したため、Atomic 要素型内のすべての SyncML コマンドが失敗しました。|
|-2016345606|0x87D101FA|Syncml(506):要求の処理中にアプリケーション エラーが発生しました。|
|-2016345607|0x87D101F9|Syncml(505):SyncML 要求メッセージで使用される指定されたバージョンの SyncML DTD を受信者がサポートしていないか、サポートすることを拒否しました。|
|-2016345608|0x87D101F8|Syncml(504):受信者がゲートウェイまたはプロキシとして機能しているときに、URI (例: HTTP、FTP、LDAP) で指定された上流の受信者、または要求を完了するためにアクセスする必要があるその他の補助的な受信者 (例: DNS) から、時間内に応答を受け取りませんでした。|
|-2016345609|0x87D101F7|Syncml(503):受信者が一時的に過負荷になっているか、メンテナンス中のため、現在要求を処理できません。|
|-2016345610|0x87D101F6|Syncml(502):受信者がゲートウェイまたはプロキシとして機能しているときに、要求を満たすためにアクセスした上流の受信者から無効な応答を受け取りました。|
|-2016345611|0x87D101F5|Syncml(501):要求を満たすために必要なコマンドが受信者でサポートされていません。|
|-2016345612|0x87D101F4|Syncml(500):受信者が予期しない状態になったため、要求を満たせませんでした|
...
|-2016345687|0x87D101A9|Syncml(425):要求されたコマンドの実行に失敗しました。送信者が受信者に対して十分なアクセス制御権限 (ACL) を持っていません。|
|-2016345688|0x87D101A8|Syncml(424):チャンクになったオブジェクトを受信しましたが、受信したオブジェクトのサイズが、1 つ目のチャンク内で宣言されているサイズと一致しませんでした。|
|-2016345689|0x87D101A7|Syncml(423):要求されたコマンドを実行できませんでした。"論理的に削除" された項目が、サーバーから既に "物理的に削除" されています。|
...
|-2016345692|0x87D101A4|Syncml(420):受信者に、残りの同期データを保存する記憶域がありません。|
...
|-2016345696|0x87D101A0|Syncml(416):要求を処理できませんでした。要求で指定されたバイト サイズが大きすぎます。|
|-2016345697|0x87D1019F|Syncml(415):メディアの種類または形式がサポートされていません。|
|-2016345698|0x87D1019E|Syncml(414):要求されたコマンドを実行できませんでした。ターゲット URI が長すぎるため、受信者が処理できないか、受け入れようとしていません。|
|-2016345699|0x87D1019D|Syncml(413):受信者が、要求されたコマンドの実行を拒否しています。要求された項目が大きすぎるため、受信者が処理できないか、受け入れようとしていません。|
|-2016345700|0x87D1019C|Syncml(412):受信者が要求されたコマンドを実行できませんでした。コマンドが不完全であるか、形式が正しくありません。|
|-2016345701|0x87D1019B|Syncml(411):要求されたコマンドには、バイト サイズまたは長さの情報を持つ Meta 要素型が付属していなければなりません。|
|-2016345702|0x87D1019A|Syncml(410):要求されたターゲットは、受信者に存在しません。その転送先 URI も不明です。|
...
|-2016345705|0x87D10197|Syncml(407):要求されたコマンドを実行できませんでした。要求元が、適切な認証情報を送信する必要があります。|
...
|-2016345709|0x87D10193|Syncml(403):要求されたコマンドを実行できませんでしたが、受信者は要求されたコマンドを理解しています。|
|-2016345710|0x87D10192|Syncml(402):要求されたコマンドを実行できませんでした。適切な支払いが必要です。|
|-2016345711|0x87D10191|Syncml(401):要求されたコマンドを実行できませんでした。要求元が、適切な認証情報を送信する必要があります。|
|-2016345712|0x87D10190|Syncml(400):要求されたコマンドを実行できませんでした。コマンドの構文が間違っています。|
|-2016345807|0x87D10131|Syncml(305):要求されたターゲットには、指定されたプロキシの URI 経由でアクセスする必要があります。|
...
|-2016345812|0x87D1012C|Syncml(300):要求されたターゲットは、要求されたターゲットの多数の代替のうちの 1 つです。|
...
|-2016345898|0x87D100D6|Syncml(214):操作は取り消されました。 SyncML コマンドは正常に完了したものの、セッション内でこれ以上コマンドは処理されません。|
