...
2. パブリッシャーの **[ソフトウェア セットアップ]** ページの **[このソフトウェアをデバイスに配布する方法]** で、 **[ソフトウェア インストーラー]** を選択して次を指定します。
   ...
   たとえば、Intune に Application.msi という名前のアプリを発行する場合は、次のようになります。![パブリッシャーのソフトウェア セットアップ ページ](./media/add-apps-for-windows-pcs-in-microsoft-intune/publisher-for-pc.png)
...
7. **Windows インストーラー** ファイルの種類のみ (.exe のみ): **[リターン コード]** ページで、アプリが管理対象 Windows PC にインストールされるときに Intune で解釈される新しいエラー コードを追加できます。
   ...
   既定では、Intune により、業界標準のリターン コードを使用して、アプリ パッケージがインストールされたかどうかが示されます。**0** (成功) または **3010** (成功、再起動が必要)。 この一覧に独自のリターン コードを追加することもできます。 リターン コードの一覧を指定し、アプリ インストールで一覧にないコードが返された場合、それはエラーとして解釈されます。
...
Windows PC にソフトウェアを展開するときのヒントとテクニックに関して詳しくは、[サポートのヒント: PC への Intune ソフトウェア配布のベスト プラクティス](https://support.microsoft.com/en-US/help/2583929)に関するブログ投稿をご覧ください。
