...
> [!NOTE]
> このトピックの情報は、Intune ソフトウェア クライアントを使用して PC として管理している Windows デスクトップにのみ適用されます。 モバイル デバイスとして登録されている Windows PC のファイアウォール設定を管理する場合は、「[Intune でエンドポイント保護設定を追加する](../protect/endpoint-protection-configure.md)」を参照してください。
