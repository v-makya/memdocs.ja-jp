...
最新バージョンの [Power BI Desktop](https://aka.ms/intune/datawarehouseapi/installpowerbi) をインストールします。 詳細については、[Power BI Desktop](https://powerbi.microsoft.com/desktop) のページを参照してください。
...
または、Microsoft Endpoint Manager admin center からアプリをインストールすることもできます。
...
過去 1 週間に登録されたデバイス数/日など、環境について知りたい情報が見つかります。 Microsoft Endpoint Management admin center のブレードから取得した Intune データ ウェアハウス Power BI レポートを使用して、Intune テナントとクライアントの集団に関する分析情報を得ることができます。 また、Intune には、データを拡張または再利用することができる機能が多数あります。 Power BI と Intune Data Warehouse API には、以下のような追加の機能があります。
