---
title: macOS デバイスに関する一般的な問題を解決する | Microsoft Docs
description: macOS デバイスの Intune での一般的な問題をトラブルシューティングします
keywords: ''
author: lenewsad
ms.author: lanewsad
manager: dougeby
ms.date: 11/06/2018
ms.topic: end-user-help
ms.prod: ''
ms.service: microsoft-intune
ms.subservice: end-user
ms.technology: ''
ms.assetid: 9ce6d87e-aa8d-4b22-99ba-866e57aeba91
searchScope:
- User help
ROBOTS: ''
ms.reviewer: elocholi
ms.suite: ems
ms.custom: intune-enduser
ms.collection: ''
ms.openlocfilehash: d675b9e97fb7a452043c862bc8997d9ff957b8e5
ms.sourcegitcommit: a77ba49424803fddcaf23326f1befbc004e48ac9
ms.translationtype: HT
ms.contentlocale: ja-JP
ms.lasthandoff: 05/27/2020
ms.locfileid: "83881181"
---
# <a name="fix-common-issues-with-your-macos-device"></a>macOS デバイスに関する一般的な問題を解決する

登録中または登録後に発生する macOS の一般的な問題をトラブルシューティングします。 発生している問題が以下にない場合、会社にサポートを求めてください。 連絡先の情報は、[会社のポータル Web サイト](https://go.microsoft.com/fwlink/?linkid=2010980)でわかります。

- [使用しているバージョンの macOS がまだサポートされていない](your-macos-version-isnt-yet-supported.md)
- [使用している macOS デバイスを更新する必要がある](you-need-to-update-your-macos-device.md)
- [使用している Jamf で管理しているデバイスが非準拠である](device-little-different-jamf.md)
- [シリアル番号を見つけることができない](how-do-i-find-the-serial-number-on-my-device-macos.md)
- [使用しているデバイスを特定するよう求められる](you-are-asked-to-identify-your-device-when-trying-to-enroll-macos.md)
- [macOS 仮想マシンの登録時にエラーが発生する](unable-to-get-macOS-device-managed.md)  
- [macOS デバイスのシリアル番号が制限されているというメッセージが表示される](unable-to-get-macOS-device-managed.md)  
- [デバイスが既に他のユーザーに割り当てられている可能性がある](unable-to-get-macOS-device-managed.md)  
